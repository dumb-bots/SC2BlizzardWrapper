...
class Game():
    async def create(self, players=[], map="", host=None, server_route=None, server_address=None, matchup=""):
        ...
        if self.human_players:
            ...
            self.host = await Server.get_server(server_route, server_address, str(port))
    ...
    async def get_replay(self, filename="Example.SC2Replay"):
        ...
        if len(self.human_players) == 2:
            results = await asyncio.gather(
                self.human_players[0].leave_game(),
        ...
        async with websockets.connect("ws://{0}:{1}/sc2api".format(self.host.address, self.host.port)) as ws:
            ...
            while self.status == "started" or self.status == "replay":
                if not self.human_players:
                    ...
                    if len(self.human_players) == 2:
                        results = await asyncio.gather(
                            self.human_players[0].advance_time(step),
                            self.human_players[1].advance_time(step)
                        )
                    else:
                        results = [await self.human_players[0].advance_time(step)]
    ...
    async def observe_replay(self, step=300, client=None, id=0):
        ...
        if id == 1:
            matchup_string = str(
                self.replay_info["races"][0]) + str(self.replay_info["races"][1])
        else:
            matchup_string = str(
                self.replay_info["races"][1]) + str(self.replay_info["races"][0])
